...
The upgrade SHALL be performed by a helper that runs with the privileges needed to replace cobble's installation and service definition. Cobble itself SHALL NOT gain those privileges. The helper SHALL accept from cobble only a release version identifier and an opaque identifier for the request, which it SHALL echo in every outcome it records for that request. It SHALL reject anything that is not a well-formed version identifier and request identifier, SHALL obtain the installer and application only from the project's official releases for that version, and SHALL verify each downloaded file against the checksum published with the release before running or installing anything.
...
- **WHEN** an upgrade to a version is requested again after an earlier attempt at the same version failed
- **THEN** the earlier attempt's outcome is not reported as the outcome of the new request
- **AND** cobble keeps waiting for the helper to act on the new request

#### Scenario: A slow installation

- **WHEN** the helper is still installing when cobble would otherwise give up waiting, including after the upgraded cobble has started
- **THEN** cobble keeps following the helper for as long as the helper may run
- **AND** a successful installation is reported as a success
