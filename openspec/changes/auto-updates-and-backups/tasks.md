...
- [x] 7.1 Implement an in-process scheduler that computes the next run in local time and survives a missed window while cobble was down; verify the next run time is correct across a daylight-saving boundary
      — `cobble/schedule.py` `Scheduler`: asyncio loop, `next_run()` from naive local time recomputed each wake (≤15min sleeps), `state_dir/schedule.json` marker drives a one-shot catch-up for a window missed while down.
...
- [x] 12.1 Verify the migration on a live LXC carrying a real world: backup taken, world relocated, server starts against the separated layout with the world intact and players able to rejoin
      ...
      — Observed: on the first M2 start after `maintenance_time` (04:00) with no prior `schedule.json`, the missed-window catch-up ran an immediate backup+restart. Spec-correct but an unexpected extra stop for a mid-day upgrade — candidate to soften (seed the marker on first run, or only catch up a recently-missed window).
