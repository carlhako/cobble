...
- [x] 10.2 Ban a connected player end to end with the live bot client and verify they are disconnected, absent from the allowlist, recorded as banned, and unable to rejoin while enforcement is on  <!-- VERIFIED by the operator with a real client. -->
- [~] 10.3 Verify the exclusion preview names the right players by banning on a server whose roster contains players absent from the allowlist  <!-- PARTIAL: confirmation gate verified live (409 confirmation_required, nothing applied, no ban recorded, enforcement unchanged). The non-empty would_exclude list needs a 2nd roster account, which the operator does not currently have; a bug will be raised if it misbehaves in future. -->
- [x] 10.4 Unban that player and verify they rejoin, and that enforcement was not changed by the unban  <!-- VERIFIED by the operator with a real client. -->
- [x] 10.5 Kick a connected player and verify the session is recorded as ended by a kick with an exact duration, and that a voluntary leave in the same session history is not  <!-- VERIFIED by the operator with a real client. -->
