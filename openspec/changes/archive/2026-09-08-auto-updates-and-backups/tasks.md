...
- [x] 12.2 Verify a successful scheduled update end to end: download occurs with the server running, the window stops and restarts it once, the new version becomes ready, and the previous version is retained
      — cobble-2. Staged an older active version (`versions/1.26.45.1` renamed to `1.20.0.1`), then `POST /api/updates/apply`. Journal: `downloading BDS 1.26.45.1` (server pid running) → `downloaded 104.1 MB` → `maintenance started: updating` → one `bedrock_server exited requested=True` → `capturing a verified pre-update backup` (recorded `1.20.0.1`) → `active version set to 1.26.45.1` → `watching … grace window` (15s) → `pruning superseded versions` → `update outcome: success — updated 1.20.0.1 -> 1.26.45.1`. `versions/1.20.0.1` retained as the rollback source; status `up_to_date`.
- [x] 12.3 Verify a failed update rolls back automatically by activating a deliberately broken version: the previous version is restored and started, the world matches the pre-update capture, and the diagnostics are visible in the interface
      — cobble-2. Local fake vendor served a broken `1.30.0.1` (`bedrock_server` that never signals readiness). `apply` → download → stop → pre-update backup → `active version set to 1.30.0.1` → start → 45s readiness timeout → `rolling back` → `reactivating 1.26.45.1` → `restoring the world from the pre-update backup` → `starting 1.26.45.1` → `quarantined version 1.30.0.1` → outcome `rolled_back`. Ended `running` on `1.26.45.1`, world marker = the pre-update value, `versions/1.30.0.1` retained. `GET /api/updates/diagnostics` returned the full captured console output (version attempted, "will never signal readiness", timeout).
- [x] 12.4 Verify the failed version is not retried on the next scheduled run, and that publishing a different version resumes normal updates
      — cobble-2. With `1.30.0.1` quarantined: `check` → `skipped: true` (`previously failed at the 'readiness' step`), `apply` → `status: skipped`. Fake vendor then published a working `1.30.0.2` (full tree) → `apply` → `success` (`1.26.45.1 -> 1.30.0.2`); pruning kept the failed `1.30.0.1` and the rollback source `1.26.45.1`, removed only `1.20.0.1`. `POST /api/updates/clear-failed {"version":"1.30.0.1"}` → `{"cleared":["1.30.0.1"]}`, `update.skipping` cleared.
- [x] 12.5 Verify a scheduled backup, its retention pruning across several runs, and a restore of a captured backup returning the world to its captured state
      — cobble-2, `COBBLE_BACKUP_RETENTION=3`. Four `POST /api/backups` in a row; journal shows `pruned 1 backup(s)` after each (oldest first); final list = the 3 newest, all restorable. Restored the oldest surviving backup (`TESTMARKER=marker2`); journal: `stopping` → `capturing the state being replaced` → `putting the backup in place` → `starting` → `restore complete`; the world's `TESTMARKER` reverted from `marker4` to `marker2`; a pre-restore safety capture was created and returned as `replaced_capture`.
- [x] 12.6 Verify backup failure handling with the destination unmounted: the server keeps running, the condition is surfaced in the interface, and the schedule still attempts the next run
      — cobble-2. `chmod 0500 /backup`; `POST /api/backups` → `ok:false, error:"backup destination is not writable"`, `run_state: running`, `maintenance: None` (the pre-flight probe caught it before any stop), `backup.unhealthy` set, BDS still running. A second attempt behaved identically (no self-disable). Restored write access → next capture `ok:true`, `backup.unhealthy` cleared.
- [x] 12.7 Verify the maintenance interlock on a live server: lifecycle actions are refused during an update, crash restart does not fire, and both resume normally afterwards
      — cobble-2, broken fake version. During the `starting <new>` step: `POST /api/server/stop` → `409 maintenance_in_progress` in **0.01s** (also `restart`). `kill -9` of the spawned server pid mid-window → journal `server exited during updating maintenance; routing exit to the operation` and **no "attempting automatic restart"** between `maintenance started: updating` and `maintenance ended: updating`; the update rolled back and started the previous version. Afterwards `stop`/`start` → `200`.
      — Found + fixed here: lifecycle calls checked the maintenance flag only *after* acquiring `_op_lock`, which a step's readiness wait holds for up to `readiness_timeout` — a Stop during an update's "starting" phase hung that long before returning 409. Now the flag is checked before the lock too (commit `18a6047`).
